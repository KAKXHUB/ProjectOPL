...
do
   ...
   spawn(function()
    ...
    local Section = Tabs.Spam:AddSection("Instant Kill")
    local Input = Tabs.Spam:AddInput("SpeedInPutInstant", {
        Title = "Speed",
        Default = 0.001,
        Placeholder = "Placeholder",
        Numeric = true, -- Only allows numbers
    ...
    local Toggle = Tabs.Spam:AddToggle("MyToggleOpenYoru", {Title = "Start Yoru", Description = "Can 0.0001", Default = false })

    if Options.MyToggleOpenYoru.Value and game.Players.LocalPlayer.Character:FindFirstChild("Yoru") and game.Players.LocalPlayer.Character["Yoru"]:FindFirstChild("AnimationController") then
        local VTCYORU = getsenv(game.Players.LocalPlayer.Character["Yoru"]:FindFirstChild("AnimationController"))["PlaceId"];
    end
    ...
    game.Players.LocalPlayer.CharacterAdded:Connect(function()
        if not Options.MyToggleOpenYoru.Value then
            return
        end
        repeat wait() until Options.MyToggleOpenYoru.Value and FindToolInInventory("Yoru") 
        local Weapon;
        if Options.MyToggleOpenYoru.Value then
            Weapon = "Yoru"

        end
        repeat
            VTCYORU = "NaN";
            VTCYORU = getsenv(game.Players.LocalPlayer.Character[Weapon]:FindFirstChild("AnimationController"))["PlaceId"];
            wait()
        until VTCYORU ~= "NaN" and VTCYORU ~= nil;
    end)

    game:GetService("RunService").Stepped:Connect(function()
        if Options.MyToggleOpenYoru.Value and game.Players.LocalPlayer.Character:FindFirstChild("Yoru") and game.Players.LocalPlayer.Character["Yoru"]:FindFirstChild("AnimationController") then
            if VTCYORU and VTCYORU ~= "NaN" then
                for _ = 1, Options.SpeedInPutInstant.Value do
                    game.Players.LocalPlayer.Character:FindFirstChild("Yoru").RequestAnimation:FireServer(tonumber(VTCYORU))
                end
            end
            if game.Players.LocalPlayer.Character.Humanoid.Health <= 0 then
                VTCYORU = "NaN"
